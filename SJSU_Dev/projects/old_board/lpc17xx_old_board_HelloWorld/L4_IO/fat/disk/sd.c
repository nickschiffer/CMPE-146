...
volatile BYTE SD_TIMER1_100HZ, SD_TIMER2_100HZ; 		/* 100Hz decrement timers */
BYTE CardType;						/* Card type flags */

inline char get_spi(void)       { SELECT(); return 1;               }
inline char release_spi(void)   { DESELECT(); rcvr_spi(); return 1; }

BYTE wait_ready(void)
{
	BYTE res;

	SD_TIMER2_100HZ = 50; /* Wait for ready in timeout of 500ms */
	rcvr_spi();
	do {
		res = rcvr_spi();
	}while ((res != 0xFF) && SD_TIMER2_100HZ);

	return res;
}

void sd_initializeCardSignals(void)
{
...
)
{
	...
	{ /* Wait for data packet in timeout of 100ms */
		token = rcvr_spi();
	} while ((token == 0xFF) && SD_TIMER1_100HZ);
	...
	{
	    spiDmaTransferBlock(buff, 512, 0);
	    buff += 512;
	}
	else
	{
...
)
{
	...
	{ /* Is data token */
	    ...
	    spiDmaTransferBlock((unsigned char*)buff, 512, 0xff);
#else
		unsigned char wc = 0;
		do
...
DSTATUS sd_initialize()
{
	...
	{ /* Enter Idle state */
		SD_TIMER1_100HZ = 100; /* Initialization timeout of 1000 msec */
		...
		{ /* SDHC */
			...
			{ /* The card can work at vdd range of 2.7-3.6V */
				...
				while (SD_TIMER1_100HZ && send_cmd(ACMD41, 1UL << 30))
					; /* Wait for leaving idle state (ACMD41 with HCS bit) */

				if (SD_TIMER1_100HZ && send_cmd(CMD58, 0) == 0)
		...
		{ /* SDSC or MMC */
			...
			while (SD_TIMER1_100HZ && send_cmd(cmd, 0)) ; /* Wait for leaving idle state */

			if (!SD_TIMER1_100HZ || send_cmd(CMD16, 512) != 0) /* Set R/W block length to 512 */
			{
				#if(DEBUG_SD_CARD)
...
void sd_timerproc(void)
{
	...
	n = SD_TIMER1_100HZ; if (n) SD_TIMER1_100HZ = --n;
	n = SD_TIMER2_100HZ; if (n) SD_TIMER2_100HZ = --n;
