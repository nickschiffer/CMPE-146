 ...
 * @ingroup BoardIO
 */
class Acceleration_Sensor : public I2C_Device_Base, public SingletonTemplate<Acceleration_Sensor>
{
    public:
        bool init();   ///< Initializes the sensor

        short getX();  ///< @returns X-Axis value
        short getY();  ///< @returns Y-Axis value
        short getZ();  ///< @returns Z-Axis value

    private:
        /// Private constructor of this Singleton class
        Acceleration_Sensor() : I2C_Device_Base(Address_AccelerationSensor)
        {
        }
        ...
        static const unsigned char mWhoAmIExpectedValue = 0x1A;
        typedef enum {
            status=0,
            X_MSB=1, X_LSB=2,
            Y_MSB=3, Y_LSB=4,
