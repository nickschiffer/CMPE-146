 ...
 */
// We have better prvListTaskWithinSingleList()
...
// We have a better function for run time stats
#if 0 //( configGENERATE_RUN_TIME_STATS == 1 )
...
/*-----------------------------------------------------------*/
	...
	register unsigned short usCount = 0U;
		...
		// Report stack in bytes not 32-bit words :
...
/*-----------------------------------------------------------*/
    ...
    unsigned short usStackRemaining;
        ...
        do
        {
            ...
            #endif
#if ( configGENERATE_RUN_TIME_STATS == 1 )
            unsigned int ulStatsAsPercentage = pxNextTCB->ulRunTimeCounter / ulTotalRunTime;
            sprintf( (char*)pcWriteBuffer+strlen((char*)pcWriteBuffer),
                    ( char * ) "%10s  %c  %u  %5u  %u %2u%%  %u\r\n",
                    pxNextTCB->pcTaskName, cStatus, ( unsigned int ) pxNextTCB->uxPriority, usStackRemaining,
                    ( unsigned int ) pxNextTCB->uxTCBNumber, ulStatsAsPercentage, (unsigned int)pxNextTCB->ulRunTimeCounter);
#else
            sprintf( (char*)pcWriteBuffer+strlen((char*)pcWriteBuffer),
                    ( char * ) "%10s  %c  %u  %5u  %u\r\n",
                    pxNextTCB->pcTaskName, cStatus, ( unsigned int ) pxNextTCB->uxPriority, usStackRemaining,
        ...
        vTaskSuspendAll();
        {
            ...
            *pcWriteBuffer = ( signed char ) 0x00;
            ...
            do
            {
                ...
                if( listLIST_IS_EMPTY( &( pxReadyTasksLists[ uxQueue ] ) ) == pdFALSE )
                {
                    prvListTaskWithinSingleList( pcWriteBuffer, ( xList * ) &( pxReadyTasksLists[ uxQueue ] ), tskREADY_CHAR, ulTotalRunTime);
                }
            }while( uxQueue > ( unsigned short ) tskIDLE_PRIORITY );

            if( listLIST_IS_EMPTY( pxDelayedTaskList ) == pdFALSE )
            {
                prvListTaskWithinSingleList( pcWriteBuffer, ( xList * ) pxDelayedTaskList, tskBLOCKED_CHAR, ulTotalRunTime);
            }

            if( listLIST_IS_EMPTY( pxOverflowDelayedTaskList ) == pdFALSE )
            {
                prvListTaskWithinSingleList( pcWriteBuffer, ( xList * ) pxOverflowDelayedTaskList, tskBLOCKED_CHAR, ulTotalRunTime);
            }
            ...
            #if( INCLUDE_vTaskDelete == 1 )
            {
                if( listLIST_IS_EMPTY( &xTasksWaitingTermination ) == pdFALSE )
                {
                    prvListTaskWithinSingleList( pcWriteBuffer, ( xList * ) &xTasksWaitingTermination, tskDELETED_CHAR, ulTotalRunTime );
                }
            }
            #endif
            ...
            #if ( INCLUDE_vTaskSuspend == 1 )
            {
                if( listLIST_IS_EMPTY( &xSuspendedTaskList ) == pdFALSE )
                {
                    prvListTaskWithinSingleList( pcWriteBuffer, ( xList * ) &xSuspendedTaskList, tskSUSPENDED_CHAR, ulTotalRunTime);
                }
            }
            #endif
        }
    ...
    static void prvGenerateRunTimeStatsForTasksInList( const signed char *pcWriteBuffer, xList *pxList, unsigned long ulTotalRunTime)
    {
        ...
        do
        {
            ...
            if( ulTotalRunTime > 0UL )
            {
                ...
                {
                    /* The task has used no CPU time at all. */
                    sprintf( (char*)pcWriteBuffer+strlen((char*)pcWriteBuffer), ( char * ) "%12s  %10u  0%%\r\n", pxNextTCB->pcTaskName, 0);
                }
                else
                {
                    /* What percentage of the total run time has the task used?
                    ...
                    if( ulStatsAsPercentage > 0UL )
                    {
                        #ifdef portLU_PRINTF_SPECIFIER_REQUIRED
                        {
                            sprintf( pcStatsString, ( char * ) "%12s  %10lu  %lu%%\r\n", pxNextTCB->pcTaskName, pxNextTCB->ulRunTimeCounter, ulStatsAsPercentage );
                        }
                        #else
                        {
                            /* sizeof( int ) == sizeof( long ) so a smaller
                            printf() library can be used. */
                            sprintf( (char*)pcWriteBuffer+strlen((char*)pcWriteBuffer), ( char * ) "%12s  %10u  %u%%\r\n", pxNextTCB->pcTaskName, ( unsigned int ) pxNextTCB->ulRunTimeCounter, ( unsigned int ) ulStatsAsPercentage );
                        }
                        #endif
                    }
                    else
                    {
                        ...
                        #ifdef portLU_PRINTF_SPECIFIER_REQUIRED
                        {
                            sprintf( pcStatsString, ( char * ) "%12s  %10lu  <1%%\r\n", pxNextTCB->pcTaskName, pxNextTCB->ulRunTimeCounter );
                        }
                        #else
                        {
                            /* sizeof( int ) == sizeof( long ) so a smaller
                            printf() library can be used. */
                            sprintf( (char*)pcWriteBuffer+strlen((char*)pcWriteBuffer), ( char * ) "%12s  %10u  <1%%\r\n", pxNextTCB->pcTaskName, ( unsigned int ) pxNextTCB->ulRunTimeCounter );
                        }
                        #endif
                    }
                }
