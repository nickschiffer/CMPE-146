...
extern "C" void * _sbrk(size_t req_bytes)
{
    ...
    if (!next_heap_ptr) {
        next_heap_ptr = (char*) ram_region_1_base;
    }

    ret_mem = next_heap_ptr;    /* Save the pointer we will return */
    next_heap_ptr += req_bytes; /* Increase our pointer */
     ...
     */
    if ((unsigned)next_heap_ptr > ((unsigned)ram_region_1_end) &&
        (unsigned)next_heap_ptr < (unsigned)ram_region_2_base
    ) {
        /* Defined by linker.  This is location after global memory space */
        extern char _pvHeapStart[];
        next_heap_ptr = _pvHeapStart;

        ret_mem = next_heap_ptr;
        next_heap_ptr += req_bytes;
    }

    /* Did we exhaust the 2nd ram region too ? */
    if (next_heap_ptr > ram_region_2_end) {
        next_heap_ptr = (char*) ram_region_2_end;
        ret_mem = 0;
    }
...
extern "C" MemoryInfoType getMemoryInfo()
{
    ...
    */
     ...
     * I do not know why these magic numbers work like this but it looks
     * like mallinfo() doesn't like when we return memory with a gap from sbrk()
     */
    if (meminfo.heapUsed > 2 * one_sram_block_size) {
        meminfo.heapUsed -= (0x1007C000 - one_sram_block_size);
    }
     ...
     */
    meminfo.systemAvailable = 0;
    if ((unsigned) next_heap_ptr <= (unsigned)ram_region_1_end) {
        meminfo.systemAvailable = one_sram_block_size + (unsigned) (ram_region_1_end - next_heap_ptr) - globalMem;
    }
    else if ((unsigned)next_heap_ptr < (unsigned)ram_region_2_end){
        /* If RAM runs out, subtracting globalMem will make it negative, so cover for that */
        int avail = (int) (ram_region_2_end - next_heap_ptr) - globalMem;
        if (avail > 0) {
            meminfo.systemAvailable = avail;
        }
    }
    return meminfo;
}
