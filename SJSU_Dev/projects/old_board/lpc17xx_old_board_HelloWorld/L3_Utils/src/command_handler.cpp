...
bool CommandProcessor::handleCommand(str& cmd, UartDev& output)
{
    bool found = false;
    cmd.trimEnd("\r\n");
    ...
    else
    {
        ...
        for(i=0; i < mCmdHandlerVector.size(); i++)
        {
            ...
            if(cmd.beginsWithWholeWordIgnoreCase(cp.pCommandStr))
            {
                ...
                cp.pFunc(cmd, output, cp.pDataParam, cp.dataParamLen);
                found = true;
                break;
            }
        }
...
void CommandProcessor::getRegisteredCommandList(UartDev& output)
{
...
void CommandProcessor::getHelpText(str& helpForCmd, UartDev& output)
{
