...
unsigned spiDmaTransferBlock(unsigned char* pBuffer, uint32_t numBytes, char isWriteOp)
{
    ...
    if(isWriteOp) {
        ...
        pDmaRxChannel->DMACCControl = numBytes | TCIE_BIT;
    }
    else {
        pDmaRxChannel->DMACCDestAddr = (uint32_t)pBuffer;
        pDmaRxChannel->DMACCControl = numBytes | DST_INCR_BIT | TCIE_BIT;
    }
    ...
    if(isWriteOp) {
        ...
        pDmaTxChannel->DMACCControl = numBytes | SRC_INCR_BIT;
    }
    else {
        pDmaTxChannel->DMACCSrcAddr = (uint32_t)(&dummyBuffer);
        pDmaTxChannel->DMACCControl = numBytes;
    }
