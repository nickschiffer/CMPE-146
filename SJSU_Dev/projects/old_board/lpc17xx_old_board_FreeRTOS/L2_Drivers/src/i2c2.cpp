...
{
    bool i2c_wires_are_pulled_high = false;
    unsigned int i2c_pin_mask = ( (1<<10) | (1<<11) );

    // So far, P0.10 and P0.11 are GPIO, make sure they are pulled HIGH externally, otherwise
    // there may be a SHORT on these wires
    i2c_wires_are_pulled_high = (i2c_pin_mask == (LPC_GPIO0->FIOPIN & i2c_pin_mask) );
