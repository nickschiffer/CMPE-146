...
/**
 * rtc_CreateTimedAlarm() returns the pointer to this structure.
 ...
 * @param freq    The frequency of the alarm  @see alarmFrequencyType
 * @note This is different from rtc_CreateTimedAlarm() because once you enable
 ...
 *
 * @code
 *      alarmTimeType my_time = { 12, 30, 0 }; // Alarm at 12:30 PM
 *      alarmTimeType *my_alarm_time = rtc_CreateTimedAlarm(my_time, &my_sem);
 *
...
/**
 * Turns off an alarm that was created by rtc_CreateTimedAlarm()
 * Nothing special here, the hour is set to 25, which will never occur.
 */
inline void rtc_TurnOffAlarm(alarmTimeType *p) { p->hour = 25; p->min = p->sec=0; }
