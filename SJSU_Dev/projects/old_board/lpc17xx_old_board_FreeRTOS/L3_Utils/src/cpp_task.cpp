...
VECTOR<scheduler_task::ptr_name_pair_type> *scheduler_task::mpNamePtrPair;
...
bool  scheduler_task::addSharedObject(const char *name, void *obj_ptr)
{
    ...
    do {
        if (!mpNamePtrPair) {
            mpNamePtrPair = new VECTOR<ptr_name_pair_type>;
        }
...
void* scheduler_task::getSharedObject(const char *name)
{
    ...
    for(unsigned int i=0; i < mpNamePtrPair->size(); i++) {
        const ptr_name_pair_type& item = mpNamePtrPair->at(i);
