...
/**
 * If non-zero, then tlm_variable_set_value() will be able to decode
 * and set a floating-point value.  This can be turned off for systems
 * that take a lot of expense of floating point sscanf()
 */
#define SUPPORT_FLOAT_VAR_SET   0
...
typedef struct {
    ...
    uint16_t elm_size_bytes; /**< Size of the variable in bytes */
    uint16_t elm_arr_size;   /**< If an array, the size of the array */
 ...
 *
 * @returns c_true upon success.  If memory allocation fails, or if another variable
 *          is registered by the same name or same memory pointer, c_false is returned.
 */
