...
static inline void flashDeselect()
{
...
*/
static inline char flashSelect()
{
    flashDeselect();
    flashDeselect();
...
DRESULT flash_ReadSector(unsigned char* pData, int sectorNum, int sectorCount)
{
    ...
    CHIP_SELECT_OP()
    {
        ...
        spi1_ExchangeEightBytes(&op[0]);

        for(int i = 0; i < sectorCount; i++) {
            spiDmaTransferBlock(pData, FLASH_SECTOR_SIZE, 0);
...
static bool flash_IsPageErased(const FlashAddressType addr)
{
    ...
    CHIP_SELECT_OP()
    {
        ...
        for(int i = 0; i < FLASH_PAGE_SIZE; i+=8) {
            spi1_ExchangeEightBytes((char*)&eightBytes);
...
static void flash_WritePage(unsigned char* pData, const FlashAddressType addr)
{
    ...
    if(flash_IsPageErased(addr))
    {
        CHIP_SELECT_OP()
        {
            ...
            spiDmaTransferBlock(pData, FLASH_PAGE_SIZE, 1);
        }
        CHIP_SELECT_OP()
        {
